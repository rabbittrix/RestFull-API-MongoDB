...
test("post /users", () => {
  return request(address)
    ...
    .then(response => {
      ...
      expect(response.body.nif).toBe("295.222.603");
      expect(response.body.password).toBeUndefined();
    })
    .catch(fail);
});

test("post /users - name mandatory ", () => {
  return request(address)
    .post("/users")
    .send({
...
test("post /users - nif no match", () => {
  return request(address)
    ...
    .then(response => {
      ...
      expect(response.body.errors[0].message).toContain("Invalid nif");
    })
    .catch(fail);
});

test("post /users - email just have", () => {
  return request(address)
    .post("/users")
    .send({
      name: "dupe",
